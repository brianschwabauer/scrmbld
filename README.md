...
SCRMBLD uses Cloudflare D1 for its database. Migrations are stored in the `migrations/` directory and are applied using Wrangler.

### Running Migrations

**Local Development** (applies to your local D1 database):

```bash
pnpm db:migrate:local
```

**Remote Production** (applies to the production D1 database):

```bash
pnpm db:migrate:remote
```

**Preview/Staging** (applies to the staging D1 database):
...
1. Create a new SQL file in the `migrations/` directory with the naming convention `NNNN_description.sql` (e.g., `0005_add_new_feature.sql`)
2. Write your SQL statements in the file
3. Run the appropriate migration command above

### Migration Files

| File                                        | Description                                         |
| ------------------------------------------- | --------------------------------------------------- |
| `0001_add-gameplay-table.sql`               | Initial gameplay tracking table                     |
| `0002_Add_newsletter_signup.sql`            | Newsletter signup functionality                     |
| `0003_auth_tables.sql`                      | User authentication tables (Better Auth)            |
| `0004_achievements_notifications_stats.sql` | Achievements, push notifications, and stats indexes |

### Notes

- Migrations are applied in order based on the numeric prefix
- Each migration runs only once (Wrangler tracks applied migrations)
- Always test migrations locally before applying to production
- The `--local` flag uses the local D1 simulator bundled with Wrangler

### Troubleshooting: "no such table" errors after migration

If you see "no such table" errors after running migrations, the dev server may be using a different local database file than where migrations were applied. This can happen when multiple database files exist in `.wrangler/state/v3/d1/miniflare-D1DatabaseObject/`.

**Solution 1**: Stop the dev server, delete the `.wrangler/state/` directory, then run migrations and restart:
...
**Solution 2**: Apply migrations directly to the active database file:

```bash
# Find the database file being used (usually the most recently modified)
ls -lt .wrangler/state/v3/d1/miniflare-D1DatabaseObject/*.sqlite

# Apply migration directly
sqlite3 .wrangler/state/v3/d1/miniflare-D1DatabaseObject/<filename>.sqlite < migrations/0004_achievements_notifications_stats.sql
```

## Push Notifications Setup

Push notifications require VAPID keys for authentication with browser push services.
...
- `VAPID_PUBLIC_KEY` - Used by the browser to subscribe to push
- `VAPID_PRIVATE_KEY` - Used by the server to sign push messages
...
NOTIFICATIONS_WORKER_URL=http://localhost:8787
```

### Production Deployment

Set the secrets in Cloudflare Workers (for main app):

```bash
echo "your_public_key" | wrangler secret put VAPID_PUBLIC_KEY
echo "your_private_key" | wrangler secret put VAPID_PRIVATE_KEY
echo "https://scrmbld-notifications.your-account.workers.dev" | wrangler secret put NOTIFICATIONS_WORKER_URL
```

### Notification Worker (Durable Object)

Push notification reminders are handled by a separate Durable Object worker that provides per-user timezone-aware scheduling. The DO uses SQLite for persistent storage and hourly alarms to check which users should receive notifications.

#### Architecture

- **Main app** → notifies DO on subscribe/unsubscribe/game-played events
- **DO worker** → maintains its own subscription database with timezone info
- **Hourly alarm** → checks each hour which users' preferred notification time has arrived
- **Per-user scheduling** → users receive reminders at their preferred hour in their local timezone

#### Deploy the notification worker:

```bash
# First, set the secrets for the notification worker
wrangler secret put VAPID_PUBLIC_KEY -c workers/notifications/wrangler.jsonc
wrangler secret put VAPID_PRIVATE_KEY -c workers/notifications/wrangler.jsonc

# Deploy the notification worker
pnpm notifications:deploy
```

#### Initialize the alarm (required after first deploy):

```bash
curl -X POST https://scrmbld-notifications.your-account.workers.dev/init-alarm
```
...
```bash
pnpm notifications:tail
```

#### Status check:

```bash
curl https://scrmbld-notifications.your-account.workers.dev/status
```

Returns subscription count, sent-today count, next alarm time, and timezone distribution.

#### Manual trigger (for testing):

```bash
curl -X POST https://scrmbld-notifications.your-account.workers.dev/trigger
```

#### How it works

1. When a user enables notifications, the main app:

   - Saves the subscription to D1 (source of truth)
   - Notifies the DO worker via `/subscribe` with user ID, push credentials, and timezone

2. When a user plays the daily puzzle, the main app:

   - Notifies the DO worker via `/played` with user ID and day
   - DO marks user as "sent today" so they won't get a reminder

3. Every hour, the DO alarm fires and:

   - Checks which subscribed users have their preferred notification hour in their timezone
   - Skips users who already played today or were already sent a notification
   - Sends push notifications to remaining users
   - Marks them as "sent today"

4. At midnight UTC, the "sent today" table is cleaned up for the new day
...
- **`GET /api/generate` (for 7-letter words)**:
  ...
  - **Core Logic**:
    ...
    2.  **Collision Avoidance**: This is a crucial step for fairness. The 8 chosen letters (7 solution + 1 extra) should not be easily re-arrangeable to form _other_ valid 7-letter words if a _different_ letter from the 8 is omitted. To achieve this:
